...
# 1. Si le script est dans le projet, utiliser son dossier
# 2. Sinon, chercher dans les emplacements courants
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
if [ -f "$SCRIPT_DIR/api/main.py" ]; then
    DIR="$SCRIPT_DIR"
elif [ -f "$HOME/code/projets/health_analyzer/api/main.py" ]; then
    DIR="$HOME/code/projets/health_analyzer"
elif [ -f "$HOME/Documents/health_analyzer/api/main.py" ]; then
    DIR="$HOME/Documents/health_analyzer"
else
    echo "Dossier health_analyzer introuvable."
    echo "Emplacements cherchés :"
    echo "  - $SCRIPT_DIR"
    echo "  - $HOME/code/projets/health_analyzer"
    echo "  - $HOME/Documents/health_analyzer"
    echo ""
...
try: os.symlink('dashboard_latest.html', 'reports/dashboard.html')
except: pass
