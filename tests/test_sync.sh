#!/bin/bash
# PerformOS — Tests rapides de synchronisation via curl
# Usage: bash tests/test_sync.sh [PORT] [TOKEN]

PORT="${1:-8765}"
TOKEN="${2:-$(grep -oP 'const\s+API_TOKEN\s*=\s*"\K[^"]+' reports/dashboard.html 2>/dev/null || echo performos)}"
BASE="http://127.0.0.1:${PORT}/api/planner"

echo "============================================"
echo "PerformOS Sync Tests (curl)"
echo "Base: ${BASE}"
